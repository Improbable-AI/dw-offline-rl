...
class LearnableBase:
    ...
    def fit(
        ...
        logger_cls: D3RLPyLogger = D3RLPyLogger,
        ...
        Args:
            ...
            shuffle: flag to shuffle transitions on each epoch. (only apply to RoundIterator)
            callback: callable function that takes ``(algo, epoch, total_step)``
                , which is called every step.
            sample_weights: transitions weights (must be normalized to 1)
    ...
    def fitter(
        ...
        Args:
            ...
            callback: callable function that takes ``(algo, epoch, total_step)``
                , which is called every step.
            sample_weights: sample weights that will be used in random iterator
        ...
        if n_epochs is None and n_steps is not None:
            ...
            LOG.debug(f"RandomIterator is selected (with weights? {sample_weights is not None}).")
        elif n_epochs is not None and n_steps is None:
            assert sample_weights is None # Sample weights are not applicable in RoundIterator
