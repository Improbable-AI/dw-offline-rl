...
from experiments import get_launch_args, sweep_with_devices, parse_task_ids, launch_jobs
...
if __name__ == '__main__':
  experiment = f"{os.path.basename(os.path.dirname(Path(__file__)))}_td3bc"
  ...
  algos_scripts = [
    {"script": "python3 TD3BC/d3rlpy_algos/train_td3bc.py",},
  ]

  samplers = [
    "uniform",
    *[f"Top-{p}" for p in [0.1, 0.2, 0.5]],
    *[f"AW-{alpha}" for alpha in ["0.1"]],
  
    *[f"uniform+DRW2MaxMinScale-K{kl_weight}_F{flow_weight}_G{flow_discount}_T{discriminator_temp}_C{discriminator_clip_ratio}_L{discriminator_lr}" for 
        kl_weight, flow_weight, flow_discount, discriminator_temp, discriminator_clip_ratio, discriminator_lr in 
        itertools.product(["0.2", "1.0"], ["5.0", "1.0", "0.1"], ["1.0"], ["1.0"], ["0.2",], ["0.0001",])],

    *[f"AW-{temp}+DRW2MaxMinScale-K{kl_weight}_F{flow_weight}_G{flow_discount}_T{discriminator_temp}_C{discriminator_clip_ratio}_L{discriminator_lr}" for 
        temp, kl_weight, flow_weight, flow_discount, discriminator_temp, discriminator_clip_ratio, discriminator_lr in 
        itertools.product(["0.1", "0.2"], ["0.2", "1.0"], ["5.0", "1.0", "0.1"], ["1.0"], ["1.0"], ["0.2",], ["0.0001",])]
  ]
  ...
  common_exp_args = [
    # INSERT COMMON ARGUMENTS HERE
  ]

  if launch_args.debug:
    common_exp_args.append("--wandb=offline")
  else:
    common_exp_args.append("--wandb=online")
