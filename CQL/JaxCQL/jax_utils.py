...
def weighted_mse_loss(val, target, weights):
    return jnp.mean(jnp.square(val - target) * weights)

def weighted_sum_mse_loss(val, target, weights):
    # NOTE: only works for one dimensional case
    return jnp.sum(jnp.square(val - target) * weights)
...
def collect_jax_metrics(metrics, names, prefix=None):
    ...
    for name in names:
        if name in metrics:
            ...
            collected[name + "_median"] = jnp.median(metrics[name])
