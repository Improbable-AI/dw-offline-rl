...
def check_cql_if_run_exists(entity, project, configs):
  keys_of_interests = [
    "env",
    "seed",
    "sampler",
    "reward_norm",
    "cql.discriminator_clip_ratio",
    "cql.discriminator_kl_penalty",
    "cql.discriminator_lr",
    "cql.discriminator_temp",
    "cql.cql_min_q_weight",
    "policy_arch",
  ]

  # Check if the specified configuration has been run
  prefixed_configs = {f"config.{key_of_interest}": configs[key_of_interest]
    for key_of_interest in keys_of_interests if not key_of_interest.startswith("cql")}
  api = wandb.Api()
  ...
  is_exist = np.any([
    np.all([old_run.config[key_of_interest] == configs[key_of_interest] for key_of_interest in keys_of_interests]) for old_run in old_runs
  ])
...
class WandBLogger(object):
    ...
    def get_default_config(updates=None):
        ...
        config.wandb_api_key = config_dict.placeholder(str) # None: you login in shell
    ...
    def __init__(self, config, variant):
        ...
        if self.config.wandb_api_key != config_dict.placeholder(str):
          wandb.login(key=self.config.wandb_api_key)

        # Check redundant runs
        if os.environ.get("DEBUG", None):
            self.check_redundant_runs()
    ...
    def check_redundant_runs(self):
        if check_cql_if_run_exists(self._variant,
                project=self.config.project):
          print("Run exists")
          sys.exit()
