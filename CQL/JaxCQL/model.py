...
class FullyConnectedFlowConserveDiscriminator(nn.Module):
    observation_dim: int
    action_dim: int
    arch: str = '256-256'
    orthogonal_init: bool = False
    ...
    def __call__(self, observations, actions):
        # State-action g(a|s)
        sa = jnp.concatenate([observations, actions], axis=-1)
        g_sa = FullyConnectedNetwork(output_dim=1, arch=self.arch, orthogonal_init=self.orthogonal_init)(sa)
        # State f(s)
        s = observations
        f_s = FullyConnectedNetwork(output_dim=1, arch=self.arch, orthogonal_init=self.orthogonal_init)(s)
        return jnp.squeeze(g_sa, -1), jnp.squeeze(f_s, -1)
...
class MeanPolicy(nn.Module):
    ...
    def setup(self):
        self.base_network = FullyConnectedNetwork(
            output_dim=self.action_dim, arch=self.arch, orthogonal_init=self.orthogonal_init
        )

    def __call__(self, observations, deterministic=False, repeat=None):
        if repeat is not None:
            observations = extend_and_repeat(observations, 1, repeat)
        mean = self.base_network(observations)
        output = jnp.tanh(mean)
