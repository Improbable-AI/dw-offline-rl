...
def update_v_weighted_sum(critic: Model, value: Model, batch: Batch,
             expectile: float, weights: jnp.ndarray) -> Tuple[Model, InfoDict]:
    actions = batch.actions
    q1, q2 = critic(batch.observations, actions)
    q = jnp.minimum(q1, q2)
    ...
    def value_loss_fn(value_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        v = value.apply({'params': value_params}, batch.observations)
        value_loss = (loss(q - v, expectile) * weights).sum()
        return value_loss, {
            'value_loss': value_loss,
            'v': v.mean(),
        }
...
def update_q_weighted_sum(critic: Model, target_value: Model, batch: Batch,
             discount: float, weights: jnp.ndarray) -> Tuple[Model, InfoDict]:
    next_v = target_value(batch.next_observations)
    ...
    def critic_loss_fn(critic_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        q1, q2 = critic.apply({'params': critic_params}, batch.observations,
                              batch.actions)                                  
        critic_loss = (((q1 - target_q)**2 + (q2 - target_q)**2) * weights).sum()
        return critic_loss, {
            'critic_loss': critic_loss,
            'q1': q1.mean(),
            'q2': q2.mean()
